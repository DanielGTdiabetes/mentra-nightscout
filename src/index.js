"use strict";
// src/index.js — Nightscout MentraOS v2.10.1-combined-eco
// SDK 2.1.18 — ROBUST + FALLBACK ENDPOINTS + HEAD-UP DISPLAY + MG/MMOL SYNC
// + SPARKLINE CHARTS + CACHING (LOCAL HISTORY) + COMBINED VIEW + STRONG ECHOS
...
// --- Dimensiones seguras para G1B (evita recortes) ---
const BMP_WIDTH = 526;
const BMP_HEIGHT = 128;

// Márgenes seguros (superior/inferior)
const SAFE_TOP = 12;
const SAFE_BOTTOM = 12;
const SAFE_LEFT = 10;
const SAFE_RIGHT = 10;

// Layout: texto a la izquierda, sparkline a la derecha
const LAYOUT = {
  padding: 8,
  text: { x: SAFE_LEFT, y: SAFE_TOP, line: 10, scale: 2 },
  spark: {
    x: Math.floor(BMP_WIDTH * 0.48),
    y: SAFE_TOP,
    width: BMP_WIDTH - Math.floor(BMP_WIDTH * 0.48) - SAFE_RIGHT,
    height: BMP_HEIGHT - SAFE_TOP - SAFE_BOTTOM
  }
};

// Umbral de puntos mínimos para dibujar sparkline
...
class NightscoutMentraApp extends AppServer {
  ...
  // Fuente 5x7 mínima (subset con lo necesario)
  FONT5x7 = (() => {
    const map = {};
  ...
  generateSparklineBitmap(history, settings, canvasW = BMP_WIDTH, canvasH = BMP_HEIGHT) {
    ...
    const ds = this.downsample(points, 64);
    ...
    // Marco de sparkline dentro de márgenes
  ...
  generateCombinedBitmap(history, lastReading, settings) {
    ...
    // Componer el texto (línea 1 grande + línea 2 pequeña)
    ...
    const line2 = `${timeStr} (${timeAgo})`;

    const s2 = LAYOUT.text.scale; // escala 2 para line1
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        const { data } = await axios.get(endpoint, {
          params, timeout: 10000, headers: { 'User-Agent': 'MentraOS-Nightscout/2.10.1-combined-eco' }
        });
  ...
  async showGlucoseDisplay(session, sessionId, settings, opts = {}) {
    ...
    try {
      ...
      if (!isAlert && settings.enable_sparkline_display && history.length >= MIN_HISTORY_FOR_SPARKLINE && mode !== 'textOnly') {
        try {
    ...
    try {
      ...
      const settingsHandler = async (settingsData) => {
        try {
          ...
          // Reiniciar bucle si cambia el intervalo
          ...
          // Reset de histórico de alertas si cambian límites/unidades
      ...
      session.events?.onHeadPosition?.(async (data) => {
        try {
          ...
          const history = this.glucoseHistory.get(sessionId) || [];
          if (s?.enable_sparkline_display && history.length >= MIN_HISTORY_FOR_SPARKLINE) {
            try {
  ...
  async persistAndEchoSettings(session, parsedSettings) {
    try {
      ...
      // Construir eco según unidad activa + toggles
      ...
      // Pausar el ciclo normal del session concreto, mostrar eco 5s y reanudar
      ...
      if (sidForSession) setTimeout(() => {
        try {
          const sd = this.sessions.get(sidForSession);
          if (sd) this.startNormalOperation(sd.session, sidForSession, sd.settings);
        } catch(_) {}
      }, 5200);
    } catch (e) {
  ...
  async checkAlerts(session, sessionId, data, settings) {
    ...
    the_display:
    {
      ...
      if (last && Date.now() - last < 600000) break the_display; // 10 min cool-down
...
console.log('🚀 Nightscout MentraOS v2.10.1-combined-eco — COMBINED VIEW + strong ECHOS + safe 526x128');
...
server.app.get('/health', (_, res) => res.json({
  ...
  version: '2.10.1-combined-eco',
  activeSessions: server.sessions.size,
  features: ['combined-bmp-526x128', 'sparkline', 'head-up', 'alerts', 'mg-mmol-sync', 'fallback-endpoints', 'strong-echo']
}));
