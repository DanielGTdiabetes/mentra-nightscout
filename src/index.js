 ...
 * - Sin polyfills ni parches a prototipos.
 * - Timers centralizados por sesión y limpieza garantizada.
 * - Limpieza completa de estado en disconnect/shutdown.
 * - Display optimizado: una sola llamada por operación.
 * - Health endpoint con timestamp y métricas básicas.
 */
  ...
  setupEventHandlers(session, sessionId, userId){
    ...
    const runSettingsHandler = async (settingsData)=>{
      try{
        ...
        // ECO feedback corto
        try{
          let dNow=null; try{ dNow = await this.getGlucoseData(settings, sessionId); await this.checkAlerts(session, sessionId, dNow, settings); }catch{}
          ...
          const stateStr = isEs? (alarmState==="low"?"Activa: BAJA":(alarmState==="high"?"Activa: ALTA":"Sin alarma"))
          ...
          if (this.canRender(sessionId, LAYERS.ECO)){ this.showText(session, sessionId, ecoTxt); this.setDisplayTimeout(sessionId, 2200, ()=> this.clearDisplay(session, sessionId)); }
        }catch{}
      }catch(_){}
    };
