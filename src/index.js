...
function createFallbackLogger(label = "AppSession") {
  const prefix = `[${label}]`;
  return {
    ...
    child(meta = {}) {
      const suffix = meta.scope || meta.component || meta.name;
      const nextLabel = suffix ? `${label}:${suffix}` : label;
      return createFallbackLogger(nextLabel);
    },
  };
}
...
const baseSessionLogger = (server?.logger && typeof server.logger.child === "function")
  ? server.logger.child({ scope: "AppSession" })
  : server?.logger || createFallbackLogger();

const defaultSessionLoggerFactory = (typeof baseSessionLogger?.child === "function")
  ? (meta = {}) => baseSessionLogger.child(meta)
  : (meta = {}) => {
      const suffix = meta.scope || meta.component || meta.name;
      return createFallbackLogger(suffix ? `AppSession:${suffix}` : "AppSession");
    };

let activeAppSession=null;
const createAppSession=(opts={})=>{
  const { logger, loggerFactory, server: providedServer, ...rest } = opts || {};
  return new AppSession({
    packageName:PACKAGE_NAME,
    apiKey:MENTRAOS_API_KEY,
    logger: logger || baseSessionLogger,
    loggerFactory: loggerFactory || defaultSessionLoggerFactory,
    server: providedServer || server,
    ...rest,
  });
};
