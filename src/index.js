...
const LAYOUT = {
  ...
  text: { x: 12, y: 28, line: 10, scale: 2 }, // fuente 5x7, escalada x2 para el valor
  spark: { x: 280, y: 22, width: 576 - 280 - 8, height: 119 } // ~288px de ancho
};
...
class NightscoutMentraApp extends AppServer {
  ...
  async getUserSettings(session) {
    try {
      ...
      const settings = {
        ...
        enable_head_up_display: enable_head_up_display === true,
        enable_sparkline_display: enable_sparkline_display === true,
  ...
  async onHeadPosition(session) {
    ...
    if (enable_head_up_display) {
      ...
      // No debe estar permanentemente activo. Se muestra, y luego se oculta.
      
      // Verifica si ha pasado el tiempo suficiente para mostrar de nuevo el HUD
  ...
  async render(session, { forceHide = false, forceShow = false } = {}) {
    ...
    // Lógica para mostrar el bitmap (se mostrará de forma permanente si no se usa forceShow)
    const now = Date.now();
    const lastShown = this.headUpLastShown.get(userId) || 0;
